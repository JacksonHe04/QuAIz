  ...
  const renderQuestionWithBlanks = () => {
    ...
    parts.forEach((part, index) => {
      ...
      if (index < parts.length - 1) {
        ...
        const isCorrect = showCorrectAnswer && userAnswer.trim().toLowerCase() === correctAnswer.toLowerCase();
        const isWrong = showCorrectAnswer && userAnswer.trim() !== '' && !isCorrect;
          ...
          <span key={`blank-${index}`} className="inline-block mx-1">
            ...
            {showCorrectAnswer && (
              <div className="text-xs mt-1">
                ...
                {isWrong && (
                  <span className="text-red-600">✗ 正确答案: {correctAnswer}</span>
                )}
                {!userAnswer.trim() && (
