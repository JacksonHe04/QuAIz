 ...
 * @param index 题目序号
 */
const createSingleChoiceQuestion = (id: string, subject: string, index: number): Question => ({
  id,
 ...
 * @param index 题目序号
 */
const createMultipleChoiceQuestion = (id: string, subject: string, index: number): Question => ({
  id,
 ...
 * @param index 题目序号
 */
const createFillBlankQuestion = (id: string, subject: string, index: number): Question => ({
  id,
 ...
 * @param index 题目序号
 */
const createShortAnswerQuestion = (id: string, subject: string, index: number): Question => ({
  id,
 ...
 * @param index 题目序号
 */
const createCodeOutputQuestion = (id: string, subject: string, index: number): Question => ({
  id,
 ...
 * @param index 题目序号
 */
const createCodeWritingQuestion = (id: string, _subject: string, index: number): Question => ({
  id,
 ...
 * @returns 生成的试卷
 */
export const mockGenerateQuiz = async (request: GenerationRequest): Promise<Quiz> => {
  // 模拟网络延迟
  await delay(2000);
...
const gradeSingleChoice = (question: Question) => {
  if (question.type !== 'single-choice') return { score: 0, feedback: '题目类型错误' };
...
const gradeMultipleChoice = (question: Question) => {
  if (question.type !== 'multiple-choice') return { score: 0, feedback: '题目类型错误' };
...
export const mockGradeQuiz = async (quiz: Quiz): Promise<GradingResult> => {
  ...
  const results = quiz.questions.map(question => {
    const grader = questionGraders[question.type as keyof typeof questionGraders];
    const { score, feedback } = grader ? grader(question) : gradeOtherQuestion(question);
