...
const GRADING_INSTRUCTIONS = {
  'single-choice': {
    name: '单选题',
    instruction: '对于单选题，如果用户选择的答案索引与正确答案索引完全一致，给满分；否则给0分。',
    scoring: '满分10分，答对得10分，答错得0分'
  },
  'multiple-choice': {
    name: '多选题',
    instruction: '对于多选题，如果用户选择的所有答案索引与正确答案索引完全一致，给满分；部分正确给部分分数；完全错误给0分。',
    scoring: '满分10分，全对得10分，部分对得5分，全错得0分'
  },
  'fill-blank': {
    name: '填空题',
    instruction: '对于填空题，逐个比较用户填写的答案与标准答案，可以适当考虑同义词、不同表达方式。每个空白按比例给分。',
    scoring: '满分10分，按空白数量平均分配，每个空白答对得相应分数'
  },
  'short-answer': {
    name: '简答题',
    instruction: '对于简答题，需要综合评估答案的准确性、完整性和逻辑性。参考标准答案，但允许合理的不同表达。',
    scoring: '满分10分，根据答案质量给分：优秀(9-10分)、良好(7-8分)、一般(5-6分)、较差(1-4分)、未答(0分)'
  },
  'code-output': {
    name: '代码输出题',
    instruction: '对于代码输出题，严格比较用户答案与正确输出，注意格式、换行、空格等细节。',
    scoring: '满分10分，输出完全正确得10分，格式有小问题得8分，部分正确得5分，完全错误得0分'
  },
  'code-writing': {
    name: '代码编写题',
    instruction: '对于代码编写题，评估代码的正确性、逻辑性、代码风格。即使实现方式不同，只要逻辑正确就应该给分。',
    scoring: '满分10分，功能完全正确且代码优雅(9-10分)、功能正确但代码一般(7-8分)、部分功能正确(5-6分)、逻辑有问题(1-4分)、未答或完全错误(0分)'
  }
} as const;
...
function generateGradingUserPrompt(quiz: Quiz): string {
  const questionsDetails = quiz.questions.map((question, index) => {
...
export function validateGradingJSON(jsonStr: string): { isValid: boolean; error?: string; result?: GradingResult } {
  try {
    ...
    if (typeof result.totalScore !== 'number' || 
      ...
      return {
        isValid: false,
        error: '批改结果JSON缺少必要字段或字段类型错误：totalScore(number), maxScore(number), results(array), overallFeedback(string)'
      };
    }
    ...
    // 验证总分计算
    const calculatedTotal = result.results.reduce((sum: number, r: { score: number }) => sum + r.score, 0);
 ...
 * 从流式输出中提取批改结果JSON
 */
export function extractGradingJSONFromStream(content: string): { json?: string; isComplete: boolean } {
  // 寻找JSON开始标记
