...
export function testTimeRecorderStateTransition() {
  ...
  if (!idleState.startTime) {
    ...
    console.log('期望行为：即使generation状态为idle，时间记录状态应该保持completed状态');
  } else {
...
export function testComponentDisplayLogic() {
  ...
  const testCases = [
    { startTime: null, status: 'idle', expected: false, desc: '无开始时间且状态为idle' },
    { startTime: null, status: 'generating', expected: true, desc: '无开始时间但状态为generating' },
    { startTime: Date.now(), status: 'idle', expected: true, desc: '有开始时间且状态为idle' },
    { startTime: Date.now(), status: 'completed', expected: true, desc: '有开始时间且状态为completed' }
  ];
  
  testCases.forEach((testCase, index) => {
    const shouldShow = !(testCase.startTime === null && testCase.status === 'idle');
    const result = shouldShow === testCase.expected ? '✅' : '❌';
    console.log(`${result} 测试${index + 1}: ${testCase.desc} - 应该${testCase.expected ? '显示' : '隐藏'}, 实际${shouldShow ? '显示' : '隐藏'}`);
  });
}
...
if (typeof window !== 'undefined') {
  (window as typeof window & { timeRecorderTest: typeof testFunctions }).timeRecorderTest = testFunctions;
