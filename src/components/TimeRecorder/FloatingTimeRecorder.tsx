...
export const FloatingTimeRecorder: React.FC = () => {
  ...
  const getStatusInfo = () => {
    switch (status) {
      ...
      case 'complete':
        return { icon: '✓', color: 'bg-green-600', textColor: 'text-green-600' };
      case 'error':
  ...
  return (
    <>
      ...
      {/* 展开的详细面板 */}
      {isExpanded && (
        <div className="fixed top-48 z-50 w-72 bg-white rounded-lg shadow-xl overflow-hidden
                     lg:right-4 right-2">
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="text-sm">
              ...
              <span className="text-gray-900">{formatTimestamp(startTime)}</span>
            </div>
            ...
            {endTime && (
              <div className="text-sm">
                ...
                <span className="text-gray-900">{formatTimestamp(endTime)}</span>
              </div>
            )}
            ...
            {status === 'complete' && duration && (
              <div className={`mt-3 p-2 rounded-md text-xs ${
                duration < 5000 ? 'bg-green-50 text-green-700' :
                duration < 15000 ? 'bg-yellow-50 text-yellow-700' :
                'bg-red-50 text-red-700'
              }`}>
                {duration < 5000 ? '⚡ 生成速度很快！' :
                 duration < 15000 ? '⏱️ 生成速度正常' :
                 '🐌 生成耗时较长，可考虑优化'}
              </div>
            )}
          </div>
